...
-   Multi-Armed Bandit:
	...
	-   ε-greedy has linear total regret. To resolve this, pick a decay schedule for <b><i>ε<sub>1</sub>, ε<sub>2</sub></i></b>, ... . However, it's not possible to use because <b><i>V*</i></b> is needed to calculate the gaps.
	...
	-   At each step, information state <i><b>S'</i></b> summarizes all information accumulated so far.
...
-   Contextual Bandits:
	...
	-   <i><b>S = P [ S ]</i></b> is an unknown distribution over states(or "contexts").
	    
	-   <i><b>R \(r\) = P [ R = r | S = s, A = a ]</i></b> is an unknown probability distribution over rewards.
    

-   MDPs:
    

	-   For unknown or poorly estimated states, replace reward function with <i><b>r<sub>max</sub></i></b>. Means to be very optimistic about uncertain states.
	    
	-   Augmented MDP: includes information state so that <i><b>S' = (S , I)</i></b>.
...
-   Model-Based RL:
	...
	-   Learning <i><b>s, a → r</i></b> is a regression problem.
	-   Learning <i><b>s, a → s'</i></b> is a density estimation problem.
...
-   Parametrize the policy: <i><b>π<sub>θ</sub> (s, a) = P [a | s, θ]</i></b>
...
-   Finite Difference Policy Gradient:
	...
	-   For each dimension in <i><b>θ</i></b> parameters, perturbing <i><b>θ</i></b> by small amount <i><b>ε</i></b>. (look at the formula)
	    
	-   Uses <i><b>n</i></b> evaluations to compute policy gradient in <i><b>n</i></b> dimension.
...
-   Monte-Carlo Policy Gradient:
	...
	-   Score function is <i><b>∇<sub>θ</sub> log π<sub>θ</sub> (s, a)</i></b>
...
-   Actor-Critic Policy Gradient:
	...
	-   Critic: updates action-value function parameters <i><b>w</i></b>.
	    
	-   Actor: updates policy parameters <i><b>θ</i></b> in direction suggested by critic.
	    
	-   Approximating the policy gradient introduces bias.
	    
	-   Using baseline function <i><b>B</i></b> to reduce variance. Value function <i><b>V</i></b> could be a good baseline.
	    
	-   Using advantage function to reduce the variance: <i><b>Q<sub>π</sub> (s, a) - V<sub>π</sub> (s)</i></b>
...
-   So far, we had <i><b>V(s)</i></b> or <i><b>Q(s, a)</i></b>. But with large scale problems, calculating them takes too much memory and time. Solution: estimate the value function with function approximation:
    
    <i><b>v' (s, w) ≈ v<sub>π</sub> (s)</i></b> or <i><b>q' (s, a, w) ≈ q<sub>π</sub> (s, a)</i></b>

	Generalize from seen states to unseen ones. Update parameter <i><b>w</i></b> using MC or TD learning.
...
-   Incremental Methods:
	...
	-   Target value function for MC is the return <i><b>G<sub>t</sub></i></b> and for TD is the λ-return <i><b>G<sup>λ <sub>t</sub></sup></i></b>
...
-   Batch Methods:
	...
	-   Least squares algorithms find parameter vector <i><b>w</i></b> minimizing sum squared error.
...
-   On-policy Learning:
	...
	-   SARSA:
		...
		-   <i><b>Q(S, A) ← Q(S, A) + ⍺ * (R + γ * Q(S', A') - Q(S, A))</i></b>
...
-   Off-policy Learning:
	...
	-   Learn about policy <i><b>π</i></b> from experience sampled from <i><b>μ</i></b>.
	    
	-   Evaluate target policy <i><b>π (a|s)</i></b> to compute <i><b>v<sub>π</sub> (s)</i></b> or <i><b>q<sub>π</sub> (s, a)</i></b> while following the behavior policy <i><b>μ (a|s)</i></b>.
	...
	-   Q-Learning:
		...
		-  <i><b> S'</i></b> is gathered using the behavior policy.
...
-   Value Iteration:
	...
	-   Goal: find optimal policy <i><b>π</i></b>
	    
	-   Start with an arbitrary value function: <i><b>v<sub>1</sub></i></b>  and apply Bellman backup to get to  <i><b>v<sub>2</sub></i></b>  and finally <i><b>v*</i></b>.
...
-   Policy Iteration:
	...
	-   Given a policy <i><b>π</i></b> (any policy works):
	    

	-   Evaluate the policy: <i><b>V<sub>π</sub> (s) = E [ R<sub>(t+1)</sub> + γ * R<sub>(t+2)</sub> + ... | S<sub>t</sub> = s ]</i></b>
	    
	-   Improve the policy: <i><b>π' = greedy ( V<sub>π</sub> )</i></b>
...
-   Policy Evaluation:
	...
	-   Start with an arbitrary value function: <i><b>v<sub>1</sub></i></b>  and apply Bellman backup to get to <i><b>v<sub>2</sub></i></b>  and finally <i><b>v<sub>π</sub></i></b>.
	    
	-   Use both synchronous and asynchronous backups.
	    
	-   Will converge.
	    
	-   <i><b>V<sup>(k+1)</sup> = R<sup>π</sup> + γ * P<sup>π</sup> * V<sup>k</sup></i></b>
    

-   Input: <i><b>MDP(S, A, P, R, γ)</i></b> - Output: optimal value function <i><b>V*</i></b> and optimal policy <i><b>π*</i></b>
...
-   Partial ordering over policies:
    ...
    <i><b>π' >= π</i></b> if <i><b>v<sub>π'</sub> (s) >= v<sub>π</sub> (s), ∀ s </i></b>

-   Action-Value function Q is the same as value function, but also takes action as input:
    
    <i><b>Q<sub>π</sub> (s, a) = E<sub>π</sub> [G<sub>t</sub> | S<sub>t</sub> = s, A<sub>t</sub> = a] = E<sub>π</sub> [R<sub>(t+1)</sub> + γ * Q<sub>π</sub> (S<sub>(t+1)</sub>, A<sub>(t+1)</sub>) | S<sub>t</sub> = s, A<sub>t</sub> = a]</i></b>

-   Policy is a distribution over actions given states:
    <i><b>π (a | s) = P [ A<sub>t</sub> = a | S<sub>t</sub> = s]</i></b>
	
	Policies are stationary:
<i><b>A<sub>t</sub> ~ π (. | S<sub>t</sub>), ∀ t > 0</i></b>


-   The returns are random samples, the value function is an expectation over there random samples.
    
-   Value function:
    <i><b>v<sub>π</sub> (s) = E<sub>π</sub> [G<sub>t</sub> | S<sub>t</sub> = s] = E<sub>π</sub> [R<sub>(t+1)</sub> + γ * v<sub>π</sub> (S<sub>(t+1)</sub>) | S<sub>t</sub> = s]</i></b>
    
    It also can be expressed using matrices:
 <i><b>v<sub>π</sub> = R<sub>π</sub> + γ * ρ<sub>π</sub> * v</i></b>

-   Total discounted reward from state <i><b>t</i></b>:
    <i><b>G<sub>t</sub> = R<sub>(t+1)</sub> + γ * R<sub>(t+2)</sub> + γ<sup>2</sup> * R<sub>(t+3)</sub> ... + γ<sup>(T-1)</sup> * R<sub>T</sub></i></b>
...
-   Reward function:
    <i><b>R = E [R<sub>(t+1)</sub> | S<sub>t</sub> = s, A<sub>t</sub> = a]</i></b>

-   Sample episodes from a MDP: different iterations over different states in each episode.
    
-   State transition probability:
    <i><b>ρ<sub>ss'</sub> = P [S<sub>(t+1)</sub> = s′ | S<sub>t</sub> = s, A<sub>t</sub> = a]</i></b>
