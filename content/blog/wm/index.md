...
$$\mathcal{L}_{\text{pixel}}=\big\lVert \hat{o}_{t+1}-o_{t+1}\big\rVert^{2}\ \ \text{over}\ \mathbb{R}^{3\times H\times W}, \qquad \mathcal{L}_{\text{latent}}=\big\lVert g_{\phi}(z_{t},a_{t})-f_{\theta}(o_{t+1})\big\rVert^{2}\ \ \text{over}\ \mathbb{R}^{d}.$$
